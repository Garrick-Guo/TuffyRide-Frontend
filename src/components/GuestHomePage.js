...
class BookStayButton extends React.Component {
    ...
    handleSubmit = async (values) => {
      ...
      try {
        ...
        message.success("Successfully book ride");
      } catch (error) {
        message.error(error.message);
      } finally {
        this.setState({
          ...
          renderItem={(item) => (
            <List.Item actions={[<CancelReservationButton onCancelSuccess={this.loadData} reservationId={item.id} />]}>
              <List.Item.Meta
                ...
                description={
                  <>
                    <Text>Start Date: {item.checkin_date}</Text>
                    <br />
                    <Text>End Date: {item.checkout_date}</Text>
                  </>
                }
              />
            </List.Item>
  ...
  render() {
    return (
      <Tabs defaultActiveKey="1" destroyInactiveTabPane={true}>
        <TabPane tab="Search Rides" key="1">
            <SearchStays />
        </TabPane>
        <TabPane tab="My Rides" key="2">
            <MyReservations />
        </TabPane>
      </Tabs>
    );
